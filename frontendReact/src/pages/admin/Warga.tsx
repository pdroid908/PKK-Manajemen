...
export default function WargaManagement() {
  ...
  const { data: items = [], isLoading: isFetchingInitial } = useQuery<WargaItem[]>({
    ...
    queryFn: async () => {
      const endpoint = filterAktifOnly ? "/api/warga/data?aktif=true" : "/api/warga/data";
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiFetch("/api/warga/add", {
        method: "POST",
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await apiFetch(`/api/warga/update/${id}`, {
        method: "PUT",
      });
  ...
  const refreshMutation = useMutation({
    mutationFn: async () => {
      await apiFetch("/api/warga/refresh", { method: "POST" });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["warga-data"] });
  ...
  const restoreMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await apiFetch(`/api/warga/restore/${id}`, { method: "PUT" });
      if (!response.ok) {
  ...
  const hardDeleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await apiFetch(`/api/warga/delete/${id}`, { method: "DELETE" });
      if (!response.ok) {
